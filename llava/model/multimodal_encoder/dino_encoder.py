...
class DinoVisionTower(BaseVisionTower):
    def __init__(self, vision_tower, args, delay_load=False):
        ...
        eg: facebook/dinov2-base-res336
        """
        ...
        #         print(f"Extracted image size {self._image_size} from passed model name '{self.vision_tower_name}' (using {parts[0]})")
    ...
    def _forward(self, images):
        ...
        with torch.set_grad_enabled(self.unfreeze_mm_vision_tower):
            ...
            # print(f"image_forward_outs shape: {image_forward_outs['last_hidden_state'].shape}")
            image_features = self.feature_select(image_forward_outs).to(images.dtype)
            # print(f"image_features shape: {image_features.shape}")
            interp_features = self.interpolate(image_features)
            # print(f"interp_features shape: {interp_features.shape}")
